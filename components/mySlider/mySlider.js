    ...
    */
    ...
    _minMove: function (e) {
      ...
      if (pagex + this.data.sliderLength >= this.data.rightValue) {
        pagex = this.data.rightValue - this.data.sliderLength + 20
      } else if (pagex <= 20) {
        pagex = 20
      }
      ...
      let lowValue = parseInt(((pagex-20) / (this.data.bigLength)) * parseInt(this.data.max - this.data.min) + this.data.min)
      this.setData({
        lowValue
      })
    ...
    _maxMove: function (e) {
      ...
      let heighValue = parseInt((pagex - (this.data.bigLength - pagex) / (this.data.bigLength - 20)*20)/ this.data.bigLength * (this.data.max - this.data.min) + this.data.min)
  ...
  ready: function () {
    ...
    util.wxPromisify(wx.getSystemInfo)()
      ...
      .then(() => {
         ...
         */
        that.setData({
            lowValue: that.data.minValue,
            heighValue: that.data.maxValue
        })
          setTimeout(() => {
