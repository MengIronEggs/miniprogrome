...
Page({
  ...
  data: {
    ...
    receiptPlanDiscount: 0,
    receiptPlanCouponsDiscount: 0,
    receiptPlanId: "",
    payMoney: 0,
    //是否已经缴费完成
    payComplete: false,
  ...
  onLoad (options) { 
    ...
    if (options.receiptPlanId){
      this.setData({
        receiptPlanId: options.receiptPlanId,
        saleContractId: options.receiptPlanId,
  ...
  getData(receiptPlanId, saleContractId) {
    getApp().globalData.$post('agenthouseCutomer/RentContractController/getPayReceiptList', {
      receiptPlanId: receiptPlanId,
      saleContractId: saleContractId,
    }).then((res) => {
      let data = res.data;
      let { contractInfo: { houseAddress, saleContractId, houseName }, receiptPlan: { couponsDiscount, discount, differencesRent, number, planRent, realRent, submitCount, receiptPlanId } } = data;

      differencesRent = differencesRent <= 0 ? 0 : differencesRent;  // planRent - realRent < 0 ? 0 : planRent - realRent;
      //realRent = planRent - (differencesRent || 0);
      ...
      this.setData({
        ...
        receiptPlanDiscount: discount,
        receiptPlanCouponsDiscount: couponsDiscount,
        payMoney: differencesRent,
        receiptPlanId: receiptPlanId,

        payComplete: !differencesRent,
        houseName: houseName
      })
      if (Number(this.payMoney)>=3000){
        wx.showModal({
          title: "温馨提示：",
          content: "请下载APP进行支付",
        })
  ...
  submit() {
    ...
    if (this.data.payComplete){
      wx.showToast({ title: "租金已经缴齐", icon: 'none' })
    ...
    getApp().globalData.$post("agenthouseCutomer/PayController/rentChargePayOrder", {
      "receiptPlanId": receiptPlanId,
      "planRent": receiptPlanPlanRent,
      "realReceipt": payMoney,
      "openId": openId,
      "receiptSubjectCode": "0057001",
      "receiptWayCode": "0058009",
      ...
      return getApp().globalData.$post("agenthouseCutomer/wxPay/payInfo", {
        "userId": userId,
        "body": nameGoods,
        "notifyURL": notifyURL,
        "outTradeNo": tradeNo,
